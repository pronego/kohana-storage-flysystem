...
class Filesystem extends FlysystemFilesystem
{
    ...
    public function prepend(string $path, string $data, string $separator = PHP_EOL)
    {
        ...
        {
            $this->write($path, $data.$separator.$this->read($path));
        }
	 ...
	 * @throws \League\Flysystem\FilesystemException
	 */
    public function append(string $path, string $data, string $separator = PHP_EOL)
    {
        if ($this->fileExists($path))
        {
            $this->write($path, $this->read($path).$separator.$data);
        }
